...
import { AlloyGenerateEventClassification, AlloyGenerateMetrics } from '../types/telemetry';
...
export async function generateComponent (type: AlloyComponentType, folder: AlloyComponentFolder, extension: AlloyComponentExtension): Promise<void> {
	...
	try {
		...
		ExtensionContainer.publicLog2<AlloyGenerateMetrics, AlloyGenerateEventClassification>('alloy.generate', { type });
...
export async function generateModel (): Promise<void> {
	...
	try {
		...
		ExtensionContainer.publicLog2<AlloyGenerateMetrics, AlloyGenerateEventClassification>('alloy.generate', { type: 'model' });
	} catch (error) {
		if (error instanceof UserCancellation) {
			return;
		}
